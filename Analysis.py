...
if climb:
    flightData = pd.read_csv("FlightData/P51D-climb.csv")
    N = flightData.shape[0]
    solutions = np.zeros((N, 2))
    consumptions = np.zeros(N)
    MachBlade = np.zeros(N)
    ...
    TAS_guess = 110
    beta_guess = np.radians(15)
    for i in range(N):
        flight = FlightData(RPM=flightData["RPM"][i],
                            MP=flightData["MP"][i],
                            power=flightData["Pengine"][i],
                            altitude=flightData["Altitude"][i],
                            TAS_measured=TAS_guess,
                            Vz=flightData["Vz"][i],
                            reduce=0.477)
                
        analysis = FlightAnalysis(aircraft, flight)

        u0_sol, beta_pitch_sol = analysis.solve2(TAS_guess, beta_guess)
        ...
        consumptions[i] = SFC/1000 # in L/h
        alphaList[i] = bem.alpha
        thetas[i] = aircraft.getTheta(flight.Vz, u0_sol)
        TAS_guess = u0_sol/0.44704
        beta_guess = beta_pitch_sol
        print(f"Point {i+1}/{N} solved.")
        print(f" u0_sol = {u0_sol:.2f} m/s, beta_pitch_sol = {np.degrees(beta_pitch_sol):.2f} deg")
        print(f" theta = {np.degrees(thetas[i]):.2f} deg")

    table = pd.DataFrame(columns=["Supercharger Mode", "RPM", "MP [inHg]", "Pengine [bhp]", "Altitude [ft]", "Vz [ft/min]", "TAS solved [mph]", "Pitch angle [deg]", "J", "Etap", "Consumption [L/h]", "Mach Blade Tip"])
    table["Supercharger Mode"] = flightData["Supercharger Mode"]
    table["RPM"] = flightData["RPM"]
    table["MP [inHg]"] = flightData["MP"]
    table["Pengine [bhp]"] = flightData["Pengine"]
    table["Altitude [ft]"] = flightData["Altitude"]
    table["Vz [ft/min]"] = flightData["Vz"]
    table["TAS solved [mph]"] = solutions[:, 0] / 0.44704
    table["thetas [deg]"] = np.degrees(thetas)
    table["Pitch angle [deg]"] = np.degrees(solutions[:, 1])
    table["J"] = J
    table["Etap"] = etaP
    table["Consumption [L/h]"] = consumptions
    table["Mach Blade Tip"] = MachBlade
    table.to_csv("output/P51D-climb-analysis.csv", index=False)
    print(table)
    np.savetxt("output/P51D-climb-alpha.txt", alphaList)
